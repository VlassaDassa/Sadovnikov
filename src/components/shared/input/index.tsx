...
function normalizeDate(date: Date, withDay: boolean): Date {
    return new Date(
        date.getFullYear(),
        date.getMonth(),
...
function formatDateValue(
    ...
    if (withDay) {
        return format(date, lang === "ru" ? "d MMMM yyyy" : "MMMM d, yyyy", {
            locale,
        });
    }
...
function parseDateValue(
    ...
    const locales = lang === "ru" ? [ru, enUS] : [enUS, ru];
    ...
    if (fullDate) {
        return normalizeDate(fullDate, withDay);
    }
    ...
    return normalizeDate(monthYear, false);
}
...
const Input: React.FC<InputProps> = ({
    ...
    const initialDate = parseDateValue(value, datePickerDay, lang);

    const initialDateInputValue = initialDate
        ? formatDateValue(initialDate, datePickerDay, lang)
        : String(value ?? "");
    ...
    const [dateInputValue, setDateInputValue] = useState(initialDateInputValue);

    const [lastValidDateValue, setLastValidDateValue] = useState(
        initialDateInputValue,
    );

    const datePickerWrapperRef = useRef<HTMLDivElement>(null);

    const dateSyncKey = JSON.stringify([
        datePicker,
        datePickerDay,
        lang,
        String(value ?? ""),
    ...
    const [previousDateSyncKey, setPreviousDateSyncKey] = useState(dateSyncKey);

    if (datePicker && dateSyncKey !== previousDateSyncKey) {
        setPreviousDateSyncKey(dateSyncKey);

        const parsedDate = parseDateValue(value, datePickerDay, lang);
        ...
        if (!parsedDate) {
            ...
            setLastValidDateValue(nextValue);
        } else {
            ...
            setLastValidDateValue(formattedValue);
        }
    }

    const commitDate = useCallback(
        (date: Date) => {
            const normalizedDate = normalizeDate(date, datePickerDay);
        ...
        [datePickerChange, datePickerDay, lang],
    );

    const commitDateInput = useCallback((): boolean => {
        const trimmedValue = dateInputValue.trim();

        if (!trimmedValue) {
            setSelectedDate(null);
            setDateInputValue("");
            setLastValidDateValue("");
        ...
        const parsedDate = parseDateValue(trimmedValue, datePickerDay, lang);

        if (!parsedDate) {
            setDateInputValue(lastValidDateValue);
    ...
    useEffect(() => {
        ...
        const animationFrame = requestAnimationFrame(() => {
            const modal = datePickerWrapperRef.current?.closest<HTMLElement>(
                "[data-modal-scroll]",
            );
    ...
    const iconOne =
        (iconPosition === "iconLeft" || iconPosition === "iconBoth") &&
        icon?.first ? (
            <Icon
                name={icon.first}
    ...
    const errorEl = error ? (
        <p className={style.inputErrorText}>⚠ {error}</p>
    ) : null;
    ...
    const inputClass = `${variant === "admin" ? style.inputAdmin : style.input} 
        ${style.additionalClass} 
        ${
            variant === "default" && iconPosition !== "noIcon"
                ? style.inputWithIcon
                : variant === "admin" && iconPosition !== "noIcon"
                  ? style.inputAdminWithIcon
                  : ""
        }  
    ...
    const counterLabel =
        counter && type === "textarea" ? (
            <div
                className={`${style.counter} ${value && value.toString().length > maxCounter! ? style.counterError : ""} ${adminLabel === "withoutLabel" ? style.counterWithoutLabel : null}`}
            >
    ...
    return (
        ...
        >
            {label && adminLabel === "withLabel" ? (
                <label
                    htmlFor={name}
                    className={`${error ? style.labelError : ""} ${style.inputLabel}`}
                >
                    {label}
                </label>
            ) : null}
                    ...
                    <input
                        ...
                        onBlur={(event) => {
                            ...
                            if (
                                ...
                                datePickerWrapperRef.current?.contains(
                                    nextTarget,
                                )
                            ) {
                                return
                        ...
                        onKeyDown={(event) => {
                            if (event.key === "Enter") {
                                ...
                                const committed = commitDateInput();
