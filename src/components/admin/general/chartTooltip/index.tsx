    ...
    if (type === "visits") {
        const metrics = payload
            ...
            .sort((firstItem, secondItem) => {
                const firstKey = String(firstItem.dataKey);

                const secondKey = String(secondItem.dataKey);

                return visitMetricOrder[firstKey] - visitMetricOrder[secondKey];
            });

        const day =
            payload.find((item) => {
                return Boolean(item.payload?.day);
            })?.payload?.day ?? String(label ?? "");

        return (
            <div className={styles.customTooltip}>
                <p className={styles.tooltipLabel}>{day}</p>
