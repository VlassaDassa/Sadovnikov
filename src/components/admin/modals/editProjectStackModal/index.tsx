    ...
    const handleChangeStack = (
        ...
        setData((prev: IProject) => ({
            ...
            stack: prev.stack.map((el) =>
                el.id === stackId ? { ...el, [field]: value } : el,
            ),
        }));
    };
        ...
        <div className={`${styles.item} modalElementBg`}>
            <div className={styles.inputs}>
                <IconUploader
                    ...
                    icon={item.icon}
                    onIconUpload={(path: string) =>
                        handleIconUpload(path, item.id)
                    }
                />
                ...
                <Input
                    ...
                    onChange={(e) =>
                        handleChangeStack(item.id, "name", e.target.value)
                    }
                />
                ...
                <div className={styles.inputsGroup}>
                    <Input
                        ...
                        maxLen={400}
                        onChange={(e) =>
                            handleChangeTooltip(item.id, "text", e.target.value)
                        }
                    />
...
const EditProjectStackModal: React.FC<EditProjectProps> = ({
    project,
    setData,
}) => {
        ...
        >
            <>
                {project?.stack.map((item) => (
                    <StackItem key={item.id} item={item} setData={setData} />
                ))}
            </>
        </ModalWrapper>
    );
