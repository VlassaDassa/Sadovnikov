...
**[Сайт](https://vlassadassa.ru) · [Figma](https://www.figma.com/design/qYYGvs1TraAVy6K7T8sjwO/Portfolio?node-id=0-1&t=b81OmKJ2Bm4Dhv0R-1) · [Архитектурные заметки](./docs/ARCHITECTURE_NOTES_RU.md)**
...
При доработке административной части Sadovnikov я использовал ИИ-агента для реализации текстового редактора и связанной с ним логики.

Изначально я обнаружил проблему в процессе реального использования собственного сайта: при заполнении страницы JustBuy стало очевидно, что обычного текстового поля недостаточно для удобного оформления большого описания проекта. А так как я считал проект уже законченным и не собирался в нём что-то менять, чтобы не стать жертвой собственного перфекционизма, а также я давно хотел попробовать использование ИИ-агента, я решил воспользоваться такой возможностью.

Я самостоятельно определил требования к редактору, выбрал желаемый UX, проверял результат работы агента, тестировал интеграцию и изучал внесённые изменения в коде.

При этом значительная часть непосредственной реализации редактора была написана ИИ-агентом. Я воспринимаю эту часть проекта как AI-assisted development, а не как код, полностью написанный вручную.

После интеграции я отдельно разобрал архитектуру решения, основные точки сохранения и отображения контента, валидацию, санитизацию HTML и логику ограничения длины текста, чтобы понимать и поддерживать эту функциональность дальше.
